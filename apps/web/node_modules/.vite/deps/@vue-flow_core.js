import { $ as createTextVNode, $n as toRefs, An as customRef, At as onBeforeMount, Bn as markRaw, Dt as mergeProps, Ft as onMounted, G as createBlock, Gn as readonly, I as Teleport, Jt as renderSlot, K as createCommentVNode, Kn as ref, M as Fragment, Mn as effectScope, Nn as getCurrentScope, Ot as nextTick, Qn as toRef, Rn as isRef, St as isMemoSame, U as computed, Ut as provide, Vn as onScopeDispose, Vt as openBlock, W as createBaseVNode, Wn as reactive, Y as createPropsRestProxy, Yn as shallowRef, Yt as resolveComponent, Zt as resolveDynamicComponent, _r as normalizeStyle, dn as useSlots, er as toValue, et as createVNode, gn as watch, hr as normalizeClass, jt as onBeforeUnmount, nr as unref, nt as defineComponent, pt as h, q as createElementBlock, qt as renderList, sn as useAttrs, ut as getCurrentInstance, vr as toDisplayString, xn as withCtx, xt as inject, zt as onUnmounted } from "./vue.runtime.esm-bundler-DQmNv2jD.js";
...
function handleConnectionChange(a, b, cb) {
	if (!cb) return;
	const diff = [];
	for (const key of a.keys()) if (!b.has(key)) diff.push(a.get(key));
...
function useEdge(id2) {
	...
	if (!edge) emits.error(new VueFlowError(ErrorCode.EDGE_NOT_FOUND, edgeId));
	return {
		id: edgeId,
		edge,
		edgeEl
...
function useZoomPanHelper(vueFlowId) {
	const viewportHelper = useViewportHelper(toReactive(useVueFlow({ id: vueFlowId })));
	return {
...
function useConnection() {
	const { connectionStartHandle: startHandle, connectionEndHandle: endHandle, connectionStatus: status, connectionPosition: position } = useVueFlow();
	return {
		startHandle,
		endHandle,
		status,
...
function useHandleConnections(params) {
	...
	watch(() => connectionLookup.value.get(`${currentNodeId.value}-${handleType.value}-${handleId.value}`), (nextConnections) => {
		if (areConnectionMapsEqual(connections.value, nextConnections)) return;
...
function useNodeConnections(params = {}) {
	...
	const lookupKey = computed(() => {
		...
		let handleSuffix = "";
		if (currentHandleType) handleSuffix = currHandleId ? `-${currentHandleType}-${currHandleId}` : `-${currentHandleType}`;
		return `${currNodeId}${handleSuffix}`;
	});
	watch(() => connectionLookup.value.get(lookupKey.value), (nextConnections) => {
		if (areConnectionMapsEqual(connections.value, nextConnections)) return;
		connections.value = nextConnections;
	}, { immediate: true });
	watch([
